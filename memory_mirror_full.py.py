...
# App Config & Paths
# -------------------------
st.set_page_config(page_title="AI Memory Mirror — Full App (Fixed)", layout="wide")
    ...
    if prefer_quality and USE_HF:
        ...
        if summ:
            try:
                ...
                pass
    # Lightweight fallback: first 2 sentences or short extract
...
# -------------------------
st.title("🪞 AI Memory Mirror — Full Application (Fixed)")
...
with tab2:
    ...
    with col2:
        ...
        uploaded_audio = st.file_uploader("Upload audio file (.wav .mp3 .m4a)", type=['wav','mp3','m4a'], key="aud1")
...
with tab4:
    ...
    if df_all.empty:
        ...
        # Determine valid ID range from DB (use actual min/max IDs)
        min_id = int(df_all['id'].min())
        max_id = int(df_all['id'].max())

        selected_id = st.number_input(
            "Enter Entry ID to view/export",
            min_value=min_id,
            max_value=max_id,
            step=1,
            value=min_id
        ...
        if selected_id is not None:
            # Ensure selected_id is int (Streamlit gives correct type but just to be safe)
            try:
                selected_id = int(selected_id)
            except Exception:
                st.warning("Invalid Entry ID.")
                selected_id = None

        if selected_id:
            row = df_all[df_all['id'] == selected_id]
            if row.empty:
                st.warning("Entry not found. It might have been deleted.")
            else:
                ...
                if st.button("Delete entry (permanent)"):
                    ...
                    conn.commit()
                    st.success("Deleted — refreshing view...")
